...
trait Other
{
	...
	public function dataView()
	{
		...
		// to fix for counter -> on array we want to count from columns count -1
		...
		/**
		 * create table header
		 * it contains table columns & names
		 */
		foreach ($colsName as $colName)
		{
			...
			// get column name
			/**
			 * the getColumnName() function defined in (config_function.php) file
			 * this function replaces (_) to space for example (column_name -> Column Name)
		...
		foreach ((array)$this->results() as $row)
		{
			...
			{
				$html .= "<tr>"; // open tr tag
				// loop all columns in a row to create <td>'s tags
